 ...
 */
export function moveSelectedForward(ctx) {
    const selectedIds = getSelectedIdsFromFullState(ctx);
    if (selectedIds.length < 1) {
        return;
    }
    const selectedObjects = selectedIds.map((id) => getObjectFromMap(ctx.objectsMap, id));
