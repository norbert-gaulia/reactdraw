...
import { redoDelete, saveCreateToUndoStack, undoCreate, undoEleOpacity, undoSvgPathColor, undoSvgPathWidth, } from "../../utils/undo";
import { calcOrientation, getStartAndEndPoint, } from "../../utils/lines";
...
const arrowTool = {
    icon: React.createElement(ArrowRightBoldIcon, null),
    id: ARROW_TOOL_ID,
    tooltip: "Arrow Tool",
    onDrawStart: (data) => {
...
export function makeArrowSvg(data, orientation) {
    const lineWidth = parseInt(data.style.lineWidth);
...
function makeArrowPath(data, pointA, pointB) {
    const [x1, y1] = pointA;
    const [x2, y2] = pointB;
    ...
    arrow.setAttribute("d", `M ${x1} ${y1} L ${x2} ${y2} L ${x2 - ax} ${y2 - ay} L ${x2} ${y2} L ${x2 - bx} ${y2 - by}`);
    return arrow;
}
