...
import { redoDelete, saveCreateToUndoStack, undoCreate, undoEleOpacity, undoSvgPathColor, undoSvgPathFill, undoSvgPathWidth, } from "../../utils/undo";
import { updateSvgPathFill, updateSvgPathStroke, } from "../../utils/updateStyles/color";
...
const diamondTool = {
    ...
    icon: React.createElement(DiamondBoldIcon, null),
    onDrawStart: (data) => {
    ...
    onResize(data, ctx) {
        scaleSvg(data.element, getBoxSize(data));
    },
    undoHandlers: {
        create: undoCreate,
        delete: redoDelete,
...
export function makeDiamondSvg(data) {
    const lineWidth = parseInt(data.style.lineWidth);
    const bounds = getBoxSize(data);
    const { width, height } = bounds;
    const newSvg = createSvg(width + lineWidth, height + lineWidth, data.style.opacity);
    const path = drawDiamondInBounds(bounds, data.style);
    newSvg.appendChild(path);
    return newSvg;
}
function drawDiamondInBounds(bounds, style) {
...
function getPathDiamondString(bounds, lineWidth) {
