...
import { expandContainer, getBoxSize, isRectBounding, makeBoundingRect, mapPointToRect, } from "../../utils";
...
const eraseTool = {
    icon: React.createElement(EraserIcon, null),
    tooltip: "Eraser tool",
    id: ERASE_TOOL_ID,
...
function addObjectToCustomState(ctx, object) {
    const state = ctx.fullState[ERASE_TOOL_ID];
    state.deletedObjects.set(object.id, object);
}
function getEraserPath(data, viewContainer) {
...
function getEraserPathDString(data, viewContainer) {
    let pathString = "";
...
function getPathPoint(curr, prev, index) {
    if (index === 0) {
