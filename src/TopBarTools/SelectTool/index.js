...
import { COLORS, SELECT_TOOL_DRAG_MIN_DISTANCE, SELECT_TOOL_ID, } from "../../constants";
import { distance, getBoxSize, getElementsThatBoundsAreWithin, isPointWithinBounds, isRectBounding, setContainerRect, } from "../../utils";
import { getSelectedIdsFromFullState, notifyTool, selectElement, selectManyElements, unselectAll, unselectEverythingAndReturnPrevious, } from "../../utils/select/utils";
...
import { getCenterPoint, makeDeleteAction, makeSureArtifactsGone, } from "../../utils/utils";
import { getRotateFromDiv } from "../../utils/select/getRotateFromDiv";
import { getPointRelativeToOther, rotatePointAroundOrigin, } from "../../utils/resizeObject";
export function alertSelected(tool, ctx) {
    const fns = tool.localState?.["subs"] ?? [];
...
const selectTool = {
    tooltip: "Select tool",
    icon: React.createElement(CursorClickIcon, { style: { transform: "translate(-2px, -1px)" } }),
    id: SELECT_TOOL_ID,
    onDrawStart: (data) => {
...
export function deletedSelected(ctx) {
    const state = ctx.fullState[SELECT_TOOL_ID];
    const selectedIds = state.selectedIds;
 ...
 */
const tryClickObject = (drawingData, ctx) => {
...
function maybeClickObject(ctx, pointA, pointB, didPressShift) {
    const clickedEle = getElementsThatSatisfyPoints(ctx.objectsMap, pointA, pointB);
    // unselect everything.
 ...
 */
function getElementsThatSatisfyPoints(renderedMap, pointA, pointB) {
...
function areAnyPointsWithin({ bounds, rotation }, ...points) {
    for (const point of points) {
        const centerOfBounds = getCenterPoint(bounds);
        const [relativeX, relativeY] = getPointRelativeToOther(point, centerOfBounds);
        const rotatedPoint = rotatePointAroundOrigin(relativeX, relativeY, -rotation);
...
const handleTryClickObject = (ctx, bounds, didPressShift) => {
...
function handleSelectIds(ctx, objectIds) {
...
function handleTrySelectObjects(currData, ctx) {
    const didPressShift = ctx.lastEvent?.shiftKey ?? false;
    const selected = ctx.fullState[SELECT_TOOL_ID].selectedIds;
    const selectedIds = [...selected];
    let elementIdsToSelect = getElementIdsInsideOfBounds(ctx.objectsMap, getBoxSize(currData), ctx);
    if (didPressShift) {
        elementIdsToSelect = new Array(...new Set([...elementIdsToSelect, ...selectedIds]));
    }
    handleSelectIds(ctx, elementIdsToSelect);
}
function getElementIdsInsideOfBounds(renderedMap, bounds, ctx) {
