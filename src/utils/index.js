...
export function makeNewBoundingDiv(relativePoint, stylesToInitialize, toolId) {
    if (!isBrowser()) {
...
export function makeNewDiv(pointX, pointY, lineWidth, toolId, zIndex) {
    const id = makeid(6);
 ...
 */
export function scaleSvg(svgEle, bounds) {
    ...
    const pathEle = svgEle.lastElementChild;
    pathEle.style.transform = `scale(${widthDiff}, ${heightDiff})`;
}
function getOriginalDimensions(svg) {
...
export function mapPointToRect(point, containerDiv, viewContainer) {
 ...
 * @param data
 */
export function expandContainer(data) {
    let didExapnd = false;
...
export function getRelativeBoxSize(data, ctx) {
 ...
 */
export function setContainerRect(data) {
...
export function getElementsThatBoundsAreWithin(renderedMap, bounds) {
...
export function isRectBounding(bounds, object) {
    return (bounds.top <= object.top &&
 ...
 */
export function isPointWithinBounds(bounds, point) {
    const [x, y] = point;
    const { width, height } = bounds;
    return (width / 2 >= x && -width / 2 <= x && height / 2 >= y && -height / 2 <= y);
}
export function distance(pointA, pointB) {
    return Math.sqrt(Math.pow(Math.abs(pointA[0] - pointB[0]), 2) +
        Math.pow(Math.abs(pointA[1] - pointB[1]), 2));
}
export function makeBoundingRect(point) {
    const [x, y] = point;
 ...
 * @returns
 */
export function addPointToBounds(bounds, point) {
...
export function getBoxSize(data) {
    const div = data.containerDiv;
...
const numRegex = /-?\d+\.*\d*/;
export function getNumFrom(str) {
