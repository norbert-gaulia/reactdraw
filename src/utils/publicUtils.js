...
import { addObject, createNewObject, deleteObjectAndNotify, getToolById, getViewCenterPoint, } from "./utils";
import { SELECT_TOOL_ID } from "../constants";
import { getSelectedDrawingObjects } from "./select/getSelectedDrawingObjects";
import { getSelectedIdsFromFullState, notifyTool, selectElement, selectManyElements, unselectAll, } from "./select/utils";
import { deletedSelected } from "../TopBarTools/SelectTool";
import textAreaTool, { addCaptureHandler, placeCaretAtEnd, setupTextAreaDiv, } from "../TopBarTools/TextAreaTool";
import { setContainerStyles } from "./updateStyles/utils";
export { deletedSelected };
export { getToolById } from "./utils";
// re-exports
export { notifyTool, selectElement, selectManyElements, unselectAll, } from "./select/utils";
export { makeNewDiv, setContainerRect } from "./index";
export { addObject, centerObject, createNewObject, getViewCenterPoint, } from "./utils";
 ...
 * @param options
 */
export function createText(ctx, options) {
 ...
 */
export function deleteObject(ctx, id) {
 ...
 * deleteSelected.
 */
export function deleteAll(ctx) {
    selectAll(ctx);
    deletedSelected(ctx);
}
export function updateSelectedObjectsStyle(ctx, key, value) {
...
export function* yieldSelectedObjects(ctx) {
    const selectedObjects = getSelectedObjects(ctx);
    for (const object of selectedObjects) {
        yield object;
    }
 ...
 */
export function createCircle(ctx, options) {
...
export async function createImage(ctx, options) {
    ...
    return new Promise((resolve, reject) => {
        ...
        if (!htmlImage) {
            throw new Error("createImage must be given either url (string) or image (HTMLImageElement) options. Neither provided.");
        }
...
export function selectAll(ctx) {
    ...
    const currentlySelected = getSelectedDrawingObjects(selectedIds, ctx.objectsMap);
...
export function getSelectedObjects(ctx) {
    const selectedIds = getSelectedIdsFromFullState(ctx);
    const currentlySelected = getSelectedDrawingObjects(selectedIds, ctx.objectsMap);
