...
export function resizeSE(data, ctx) {
    ...
    const [dxWithAspectRatio, _y] = forcePreserveAspectRatio([dX, dY], data, aspectRatioES);
    const aspectRatioSE = getAspectRatioInput(data, ctx, "SE");
    const [_x, dyWithAspectRatio] = forcePreserveAspectRatio([dX, dY], data, aspectRatioSE);
    return unifiedResizeFunction(data, [dxWithAspectRatio / 2, -dyWithAspectRatio / 2], [dxWithAspectRatio, dyWithAspectRatio]);
}
