...
function startResizing(ctx, relativePoint, resizeFn) {
...
function startResize(ctx, relativePoint) {
    ctx.fullState[SELECT_TOOL_ID].prevPoint = relativePoint;
    pushResizeToUndoStack(ctx);
}
function stopResize(ctx) {
    const state = ctx.fullState[SELECT_TOOL_ID];
    state.prevPoint = null;
}
export default function addHandlersToCornerButton(buttonEle, objectId, ctx, resizeFn) {
    const handleStartResizeMouse = function (e) {
        e.stopPropagation();
    ...
    const handleResizeTouch = function (e) {
        e.stopPropagation();
        const startPoint = getTouchCoords(e);
        const relativePoint = getRelativePoint(startPoint, ctx.viewContainer);
        startResizing(ctx, relativePoint, resizeFn);
    };
    const handleStopResizeTouch = function (e) {
        e.stopPropagation();
        stopResize(ctx);
...
function getPointDiff(pointA, pointB) {
    return [pointA[0] - pointB[0], pointA[1] - pointB[1]];
}
function pushResizeToUndoStack(ctx) {
    const state = ctx.fullState[SELECT_TOOL_ID];
