...
import { CORNER_BUTTON_PRE, ROTATE_BUTTON_PRE, ROTATE_BUTTON_STYLES, ROTATE_DOTTED_LINE_STYLES, SELECTED_BUTTON_HEIGHT, SELECTED_BUTTON_WIDTH, SELECTED_CORNER_BUTTON, SELECT_FRAME_DIV_STYLES, SELECT_FRAME_PRE, } from "./constants";
...
export function selectManyElements(selectObjects, ctx) {
...
function makeSelectFrameDiv(data) {
...
function isElementSelected(div) {
    return div.querySelector('[id^="select-frame"') !== null;
}
export function unselectEverythingAndReturnPrevious(ctx) {
...
export function notifyTool(tools, data, ctx) {
...
export function unselectAll(selectedObjects, ctx) {
    for (const obj of selectedObjects) {
        unselectElement(obj, ctx);
    }
}
export function selectElement(data, ctx) {
    const selectFrame = makeSelectFrameDiv(data);
    //   console.log(ctx);
    if (selectFrame) {
        addHandlersToSelectFrame(selectFrame, data.id, ctx);
...
function addToolsToSelectionDiv(div, data, ctx) {
    const eleId = data.id;
    // corner/side buttons
...
function createButtonWithOffsets(eleId, top = 0, right = 0, bottom = 0, left = 0) {
    const button = document.createElement("button");
    setStyles(button, SELECTED_CORNER_BUTTON);
    let buttonId = "";
    if (top === 2) {
...
export function getSelectedIdsFromFullState(ctx) {
