...
import { CUSTOM_DATA_ORIENT_KEY, CUSTOM_DATA_ORIGINAL_BOUNDS, } from "../constants";
 ...
 * @returns
 */
export function serializeObjects(serializers, ctx) {
...
export function serializeSvgSetup(obj) {
    const containerState = getContainerState(obj);
...
export function serializeDomSetup(obj) {
    const containerState = getContainerState(obj);
    const customData = serializeCustomData(obj);
    return { containerState, customData };
}
export const serializeFreeDraw = (obj) => {
    const { containerState, customData } = serializeSvgSetup(obj);
    const objectCopy = { ...obj, containerDiv: containerState, customData };
    return objectCopy;
};
export const serializeCustomData = (obj) => {
    const customData = {};
...
export const serializeSquare = (obj) => {
    const { containerState, customData } = serializeDomSetup(obj);
    const objectCopy = { ...obj, containerDiv: containerState, customData };
    return objectCopy;
};
export const serializeLine = (obj) => {
    const { containerState, customData } = serializeSvgSetup(obj);
    const objectCopy = { ...obj, containerDiv: containerState, customData };
    return objectCopy;
};
export const serializeText = (obj) => {
    const { containerState, customData } = serializeDomSetup(obj);
 ...
 * @param ctx
 */
export function deserializeData(stringifiedData, deserializers, ctx, shouldAddToCanvas = true) {
    const toArray = JSON.parse(stringifiedData);
    const results = [];
...
function toolNotFoundError(object, serializers) {
    throw new Error("toolid not function in serializers. id: " +
        object.toolId +
        " .options: " +
...
export function deserializationSetup(obj) {
    const freeDrawObj = obj;
    ...
    const { id, div } = makeNewDiv(p[0], p[1], lineWidth, freeDrawObj.toolId, zIndex);
    const drawingData = {
...
function addToContainer(data, ele) {
    data.containerDiv.innerHTML = "";
    data.containerDiv.appendChild(ele);
    data.element = ele;
}
function applyRotation(ele, containerState) {
    ele.style.transform = `rotate(${containerState.rotation}deg)`;
}
export const deserializeFreeDraw = (obj, ctx, shouldAddToCanvas = true) => {
    const { drawingData, div, containerState } = deserializationSetup(obj);
    const originalBounds = drawingData.customData.get(CUSTOM_DATA_ORIGINAL_BOUNDS);
    const bbox = containerState.bbox;
    setDivToBounds(div, originalBounds);
    if (shouldAddToCanvas) {
        addObject(ctx, drawingData);
    }
    const newSvg = createSvg(originalBounds.width, originalBounds.height, drawingData.style.opacity);
...
export const deserializeSquare = (intermedObj, ctx, shouldAddToCanvas) => {
    const { drawingData, containerState, div } = deserializationSetup(intermedObj);
...
export const deserializeCircle = (obj, ctx, shouldAddToCanvas) => {
...
export const deserializeDiamond = (obj, ctx, shouldAdd) => {
    const { drawingData, div, containerState } = deserializationSetup(obj);
    const originalBounds = drawingData.customData.get(CUSTOM_DATA_ORIGINAL_BOUNDS);
    const bbox = containerState.bbox;
    setDivToBounds(div, originalBounds);
...
export const deserializeLine = (obj, ctx, shouldAdd) => {
    const { drawingData, div, containerState } = deserializationSetup(obj);
...
export const deserializeTextArea = (obj, ctx, shouldAdd) => {
    const { drawingData, containerState, div } = deserializationSetup(obj);
...
export const deserializeArrow = (obj, ctx, shouldAdd) => {
    const { drawingData, div, containerState } = deserializationSetup(obj);
    const orientation = drawingData.customData.get(CUSTOM_DATA_ORIENT_KEY);
    const bbox = containerState.bbox;
    setDivToBounds(div, bbox);
...
function deserializeCustomData(customData) {
    const data = new Map();
